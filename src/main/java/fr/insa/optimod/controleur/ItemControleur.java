...
public class ItemControleur {
    ...
    void deleteItem(MouseEvent event) {
        Couleur.supprimerCouleur(item.getId());
        pointsControleur.supprimerLivraison(Long.parseLong(item.getAdresse()));
    }

    public void setData(Item item) {
        this.item = item;
        if (item.getEstPickup()) {
        ...
        coloredTag.setStyle("-fx-background-color: " + Couleur.getHexaCouleur(item.getId()) + ";");
